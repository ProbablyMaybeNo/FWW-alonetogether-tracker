...
export default function CampaignLobby({ onEnterCampaign, onSolo }) {
  ...
  const [newCampaign, setNewCampaign] = useState(null) // { id, name, invite_code }
  ...
  // Last campaign from localStorage
  ...
  return (
    <div style={s.root}>
      <div style={s.card}>
        ...
        {/* Rejoin last campaign */}
        {lastCampaign && !campaigns.find(c => c.id === lastCampaign.id) === false && lastCampaign && (
          <div style={s.section}>
            <div style={s.sectionTitle}>RECENT</div>
            <button
              style={s.rejoinBtn}
              onClick={() => enterCampaign(lastCampaign.id)}
              type="button"
            >
            ...
            <div style={s.campaignList}>
              {campaigns.map(c => (
                ...
                >
                  ...
                  <span style={s.campMeta}>Phase {c.phase} · Round {c.round}</span>
                </button>
              ))}
            </div>
          )}
        ...
        {/* New campaign invite code */}
        {newCampaign && (
          <div style={s.panel}>
            <div style={s.panelTitle}>CAMPAIGN CREATED</div>
            <div style={s.muted}>Share this invite code with your party:</div>
            ...
            <button
              ...
              onClick={() => {
                ...
                setPanel(null)
                setCreateName('')
                onEnterCampaign(newCampaign.id)
              }}
              type="button"
            >
              ENTER CAMPAIGN
        ...
        {/* Solo */}
        <button style={s.soloBtn} onClick={onSolo} type="button">
          SOLO PLAY (LOCAL ONLY)
        </button>
...
const s = {
  ...
  logo: {
    ...
    fontWeight: 'bold',
    letterSpacing: '0.4em',
    textShadow: '0 0 20px var(--color-pip)',
    lineHeight: 1,
  },
  subtitle: {
  ...
  inviteCode: {
    ...
    letterSpacing: '0.4em',
    textAlign: 'center',
    textShadow: '0 0 20px var(--color-amber)',
    padding: '0.5rem 0',
